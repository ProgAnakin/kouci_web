      ...
      <div className="container-content">
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between">
          ...
          <nav aria-label="Social" className="flex flex-wrap items-center gap-x-5 gap-y-3">
            {socials
              .filter((s) => s.enabled && s.href && s.href !== '#')
              .map((s) => {
                const Icon = s.icon
                return (
                  <a
                    ...
                    aria-label={`Kouci on ${s.name}`}
                    className="inline-flex rounded-md p-1.5 text-silver transition-colors hover:text-brand-light focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-light"
                  >
